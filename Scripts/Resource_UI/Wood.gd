extends Control
export(String) var Name = "Wood"
export(String) var Button_Down = "Choping"
export(String) var Button_Up = "Chop"

export(float) var Current_Amnt = 0 setget set_Current_Amnt,get_Current_Amnt
export(int) var Max_Amnt = 500 setget set_Max_Amnt,get_Max_Amnt
...
export(NodePath) var Bar_ = NodePath("1/2/Inv_Bar")
...
func Update():
	...
	get_node(Amnt_).set_text("%d/%d"%[Current_Amnt,Max_Amnt])
	get_node(Rate_).set_text("+%d.0/sec"%[Current_Rate])
	get_node(Bar_).set_max(Max_Amnt)
	get_node(Bar_).set_value(Current_Amnt)
	
func calc_Amnt(delta):
	time += delta
	if (time <= 1):
		if (Current_Amnt < Max_Amnt):
			Current_Amnt = Current_Amnt +(Current_Rate * delta)
		else:
			Current_Amnt = Max_Amnt
	else:
		time = 0
